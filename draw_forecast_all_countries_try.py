...
def _load_data_from_csv(run_dir: str, method_tag: str, horizon: int):
    ...
    # 🔴 关键修改：直接按字符串提取日期，不进行 pd.to_datetime 解析
...
def plot_combined_forecast_3x3(
    ...
    for idx, country in enumerate(DEFAULT_COUNTRIES):
        ...
        for method_tag, method_label in DEFAULT_METHODS:
            ...
            # 🔴 关键修改：利用 np.arange 强行作为 X 轴坐标
            x_pred = np.arange(len(m_pred))
            
            # 1. 绘制 True Data 和红色分界线
            ...
            # 2. 根据不同数据类型设定线型
            if "_aug" in method_tag:
                ls = "-"      # aug用实线
            elif "_comb" in method_tag:
                ls = "--"     # comb用长虚线
            else:
                ls = "-."     # real及其他用点划虚线
            ...
            line_pred, = ax.plot(x_pred, m_pred, label=method_label, 
                                 color=color, linewidth=1.5, linestyle=ls, alpha=0.9, zorder=5)
...
if __name__ == "__main__":
    plot_combined_forecast_3x3(
        ...
        horizon=4,
        seq_len=4,
        out_path="./test_results/montages/all_countries_forecast_step4_try.png"
    )