...
foreach ($seq_len in $seq_lengths) {
    Write-Host "Starting Incidenza: Running $model_name on combined Italy ILI dataset with seq_len=$seq_len ..." -ForegroundColor Cyan
    ...
    & "C:\Users\Douhan\anaconda3\envs\ltsf-gpu\python.exe" -u run_longExp.py `
           ...
           --model_id "combined_Italy_ili_MS_uncertainty_${seq_len}" `
           --model "$model_name" `
           --data custom `
           --features S `
           --target incidenza `
           ...
           --use_gpu True `
           --gpu 0 `
           *> "logs/LookBackWindow/${model_name}_combined_Italy_ili_MS_incidenza_uncertainty_${seq_len}.log"
