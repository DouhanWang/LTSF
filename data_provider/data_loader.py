...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if 'italia_17_25_ILI.csv' in self.data_path:
            ...
            df_raw = df_raw[['date'] + numeric_cols]

        '''
        df_raw.columns: ['date', ...(other features), target feature]
        '''
        # The rest of the function can now proceed as normal
        cols = list(df_raw.columns)
        if self.features == 'S':
            cols.remove(self.target)
        ...
        if self.features == 'M' or self.features == 'MS':
            df_raw = df_raw[['date'] + cols]  # This will now work
            cols_data = df_raw.columns[1:]
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            df_raw = df_raw[['date'] + cols + [self.target]]
        ...
        if self.scale:
            train_data = df_data[border1s[0]:border2s[0]]
            self.scaler.fit(train_data.values)  # This will now work
            data = self.scaler.transform(df_data.values)
        else:
            data = df_data.values

        df_stamp = df_raw[['date']][border1:border2]  # This will now work
        df_stamp['date'] = pd.to_datetime(df_stamp.date)
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp.date.apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp.date.apply(lambda row: row.day, 1)
